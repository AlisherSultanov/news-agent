...
RSS_SOURCES = [
    ...
    {"name": "Kinopoisk", "url": "https://www.kinopoisk.ru/rss/news.rss"},
...
def build_sources():
    ...
    for query in SEARCH_QUERIES:
        ...
        sources.append({
            "name": f"Google: {query}",
            "url": f"https://news.google.com/rss/search?q={q}&hl=uz&gl=UZ&ceid=UZ:uz"
        })
    return sources
...
def collect_news(max_per_source=3):
    ...
    print(f"Jami manbalar: {len(sources)}")
    print("-" * 50)
...
def format_for_agent(news_list):
    result = f"Yangiliklar yigindi: {datetime.now().strftime('%d.%m.%Y %H:%M')}\n"
    result += "=" * 50 + "\n\n"
    for i, news in enumerate(news_list, 1):
        result += f"{i}. [{news['source']}]\n"
        result += f"Sarlavha: {news['title']}\n"
        if news['summary']:
            result += f"Tavsif: {news['summary'][:300]}...\n"
